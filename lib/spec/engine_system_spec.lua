...
describe('Engine', function()
    ...
    setup(function()
        ...
        function UpdateSystem:onRemoveEntity()
            self.entitiesRemoved = self.entitiesRemoved + 1
        end
    ...
    it('Entity:activate() & Entity:deactivate() works', function()
        ...
        assert.are.equal(count(updateSystem.targets), 1)
        assert.are.equal(entity.active, true)
        entity:deactivate()
        assert.are.equal(count(updateSystem.targets), 0)
        assert.are.equal(entity.active, false)
        entity:activate()
        assert.are.equal(count(updateSystem.targets), 1)
        assert.are.equal(entity.active, true)
    end)

    it('Entity:getEngine() returns engine registered to', function()
        engine:addEntity(entity)
        assert.are.equal(entity:getEngine(), engine)
    end)
